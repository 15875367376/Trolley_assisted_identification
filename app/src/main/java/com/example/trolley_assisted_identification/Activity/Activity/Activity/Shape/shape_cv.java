...
public class shape_cv {
    public static  boolean phobne = true;
    public String res = "";
    public static String TAG="ShapeDetect";
    public static String TAGtest = "testColor";
    public static String TAGWhiteOrBlack = "whiteorblack";
    ...
    public static Mat BitmapToMat(Bitmap bmp) {
        if (bmp == null){
            ...
            Mat mat = new Mat(bmp.getHeight(), bmp.getWidth(), CvType.CV_32FC1);
            Utils.bitmapToMat(bmp, mat);
            return mat;
    }

    public static Mat BitmapToMatCV8U(Bitmap bmp) {
        Mat mat = new Mat(bmp.getHeight(), bmp.getWidth(), CvType.CV_8U);
        Utils.bitmapToMat(bmp, mat);
        return mat;
    }
    ...
    public static Bitmap MatToBitmap8888(Mat mat)
    {
...
//        Mat mRgba=read_pic(false,"plate1.jpg",1);
        ...
        Bitmap bitmap1 = Bitmap.createBitmap(binary.width(),binary.height(),Bitmap.Config.ARGB_8888);
        Utils.matToBitmap(binary,bitmap1);
        ...
        if (hierarchy!=null) {
//            MainActivity.img2.setImageBitmap(MatToBitmap(binary));
        }
        double maxArea = 0;
...
//                      MainActivity.img1.setImageBitmap(MatToBitmap(result));
                        ...
                        Log.d("TAG","白色个数: " + black_white_pixle_num);
                        //白底的白色像素比较多
                        if (black_white_pixle_num>40000){

                            black_white_Flag=true;//白底
                            res+='\n'+"  -------白底------ "+'\n';
                            Log.e(TAGWhiteOrBlack,"白色个数"+black_white_pixle_num);
        ...
        if (result != null) {
            ...
            //show_bitmap(hsv_img);
            MainActivity.img2.setImageBitmap(MatToBitmap(hsv_img));
            //浅蓝色0阈值分割
            ...
            Imgproc.erode(purple,purple,kernel);
        //    MainActivity.img2.setImageBitmap(MatToBitmap(purple));
          ...
          yanse(red,6);//红色6颜色数量和该颜色对应的图形
            ...
            Mat bules = new Mat(blue,rects);
//            MainActivity.img3.setImageBitmap(MatToBitmap(bules));
            yanse(blue,4);
            //青色5阈值分割
            Mat cyan = new Mat();
...
//
//            res += '\n'+"浅蓝色个数："+Cambridge_blue_Num+"黄色个数："+yellow_Num+"品红色个数:"+mag_Num
//                    +"蓝色个数："+blue_Num+"青色个数:"+qing_Num+"红色个数:"+red_Num+'\n';
            ...
            if (black_white_Flag==false){
                ...
                //修改此处可 处理缺少图像的问题
                Imgproc.Canny(canny_gray,canny,20,40);//边缘化二值化检测
         //       MainActivity.img1.setImageBitmap(MatToBitmap(canny));
                Mat canny_kernel=Imgproc.getStructuringElement(Imgproc.MORPH_ELLIPSE,new Size(1,1));//  指定腐蚀膨胀核
                Imgproc.morphologyEx(canny,canny,Imgproc.MORPH_ERODE,canny_kernel,new Point(-1,-1),5);//闭操作，先膨胀后腐蚀清楚小黑点，清楚连通区域
               ...
               // MainActivity.img2.setImageBitmap(MatToBitmap(canny));
                Imgproc.morphologyEx(canny,canny,Imgproc.MORPH_CLOSE,canny_kernel2);//闭操作，先膨胀后腐蚀清楚小黑点，清楚连通区
      ...
      //          MainActivity.img1.setImageBitmap(MatToBitmap(canny));
            ...
            Imgproc.findContours(resutl_mask, resutl_contours, canny_all_hierarchy,
                    Imgproc.RETR_LIST, Imgproc.CHAIN_APPROX_SIMPLE);//边缘化之后再查找所有的轮廓
      ...
      //     MainActivity.img2.setImageBitmap(MatToBitmap(canny_new_img));
            ...
            for (MatOfPoint c : resutl_contours) {
                ...
                //识别面积 消除误检
                if (area > 4000&&area<300000&&!black_white_Flag) {

                    DetectShape(canny_new_img,new MatOfPoint2f(c.toArray()));//统计形状个数

                }
                else if (area > 5000&&area<30000)
                {
                    DetectShape(canny_new_img,new MatOfPoint2f(c.toArray()));//统计形状个数
                }
            }
           ...
           MainActivity.img2.setImageBitmap(MatToBitmap(canny_new_img));
            ...
            res +='\n'+"三角形个数："+(san_Num)+"矩形个数："+(rect_Num)+"菱形个数："+(lin_Num)
                    + "五角形个数："+(star_Num)+"圆形个数："+(yuan_Num);//白底
            MainActivity.text1.setText(res);
//            MainActivity.Connect_Transport.sendShape("A"+rect_Num+"B"+yuan_Num+"C"+san_Num+"D"+lin_Num+"E"
//            +star_Num);
...
//        }
    ...
    public static Mat DetectShape(Mat canny_new_img,MatOfPoint2f c1){
     ...
     //       MainActivity.img2.setImageBitmap(MatToBitmap(canny_new_img));
        ...
        if (approx.toList().size()==3){
            if(phobne) {
                Imgproc.putText(canny_new_img, "san", new Point(cx, cy), Core.NORM_INF, 2, new Scalar(255, 0, 0), 3);
            }else {

                Imgproc.putText(canny_new_img, "san", new Point(cx, cy), Core.NORM_INF, 8, new Scalar(255, 0, 0), 8);
            } san_Num++;
        }
        ...
        //如果有四个顶点，则是正方形或者长方形
            ...
            //if (minArea-area>area*0.2)
            {

                rect_Num++;
                if (phobne){
                Imgproc.putText(canny_new_img, "Ju", new Point(cx, cy),
                        Core.NORM_INF, 2, new Scalar(255, 0, 0), 3);
                }
                else {
                    Imgproc.putText(canny_new_img, "Ju", new Point(cx, cy),
                            Core.NORM_INF, 8, new Scalar(255, 0, 0), 8);
                }
                ...
                lin_Num++;
                if (phobne) {
                Imgproc.putText(canny_new_img, "ling", new Point(cx, cy),
                       Core.NORM_INF, 2, new Scalar(255, 0, 0), 3);
                }
                else {
                    Imgproc.putText(canny_new_img, "ling", new Point(cx, cy),
        ...
        //除了以上情况之外，我们假设为圆形
            ...
            if (phobne) {
            Imgproc.putText(canny_new_img, "circle", new Point(cx, cy),
                    Core.NORM_INF, 3, new Scalar(255, 0, 0), 5);
            } else {
                Imgproc.putText(canny_new_img, "circle", new Point(cx, cy),
                        Core.NORM_INF, 8, new Scalar(255, 0, 0), 8);
            }
    ...
    public Mat yanse(Mat mask,int i){
        ...
        for (MatOfPoint c : contour) {
            int area = 0;
            area = (int) Imgproc.contourArea(c);
            Log.i(TAGtest, "代号 :"+i+"  颜色形状识别各个轮廓的面积："+area);
